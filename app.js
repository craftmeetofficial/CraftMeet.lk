...
// 🔑 Browser-safe Global Router Endpoint එකක් පාවිච්චි කර CORS Error එක මඟහැරීම
...
async function sendAMessageToAIBox() {
    ...
    // Mistral Prompt Format එකට සරිලන සේ සරල පෞරුෂත්ව Prompt එකක් සැකසීම
    ...
    try {
        // 3. Hugging Face Serverless API එක හරහා Call එකක් දීම
        ...
        // AI එකෙන් එන දත්ත බ්‍රවුසරයට ගැළපෙන සේ Filter කිරීම
        if (Array.isArray(result) && result[0]?.generated_text) {
            aiReplyText = result[0].generated_text.trim();
        } else if (result && result.error) {
            // පළමු වතාවේදී Model එක load වෙන්න තත්පර කිහිපයක් ගතවිය හැක (Cold Start)
